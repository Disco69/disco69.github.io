import { UserPlan, Income, Expense, Goal, Frequency, ExpenseCategory, Priority, GoalCategory } from '@/types';
  ...
  try {
    ...
    if (data.metadata && data.userPlan) {
      ...
      if (exportData.metadata.appName !== 'Finance Planner') {
        result.warnings.push('File was not exported from Finance Planner - compatibility not guaranteed');
      }
    ...
    // Check if it's a direct UserPlan object
      ...
      result.errors.push('Invalid file format - expected Finance Planner export or UserPlan object');
    }
 ...
 */
export function parseCSVImport(csvContent: string, options: CSVImportOptions = DEFAULT_CSV_IMPORT_OPTIONS): ImportResult {
  const result: ImportResult = {
    success: false,
    errors: [],
    warnings: []
 ...
 */
function parseIncomeFromCSV(rows: string[][], headers?: string[], result?: ImportResult): Income[] {
  const income: Income[] = [];
  
  // Define expected column indices
  const columnMap = mapCSVColumns(headers, [
    'ID', 'Name', 'Amount', 'Frequency', 'Description', 
    'Start Date', 'End Date', 'Is Active', 'Created At', 'Updated At'
  ]);
 ...
 */
function parseExpensesFromCSV(rows: string[][], headers?: string[], result?: ImportResult): Expense[] {
  const expenses: Expense[] = [];
  
  const columnMap = mapCSVColumns(headers, [
    'ID', 'Name', 'Amount', 'Category', 'Due Date', 'Recurring', 
    'Frequency', 'Description', 'Priority', 'Is Active', 'Created At', 'Updated At'
  ]);
  ...
  for (const row of rows) {
    try {
      const expenseItem: Expense = {
        ...
        name: row[columnMap.Name] || '',
        amount: parseFloat(row[columnMap.Amount]) || 0,
        category: (row[columnMap.Category] as ExpenseCategory) || ExpenseCategory.MISCELLANEOUS,
        dueDate: row[columnMap['Due Date']] || new Date().toISOString(),
        recurring: parseBoolean(row[columnMap.Recurring]),
        frequency: row[columnMap.Frequency] ? (row[columnMap.Frequency] as Frequency) : undefined,
        description: row[columnMap.Description] || undefined,
 ...
 */
function parseGoalsFromCSV(rows: string[][], headers?: string[], result?: ImportResult): Goal[] {
  const goals: Goal[] = [];
  
  const columnMap = mapCSVColumns(headers, [
    'ID', 'Name', 'Target Amount', 'Current Amount', 'Target Date', 
    'Description', 'Category', 'Priority', 'Is Active', 'Created At', 'Updated At'
  ]);
  ...
  for (const row of rows) {
    try {
      const goalItem: Goal = {
        ...
        description: row[columnMap.Description] || undefined,
        category: (row[columnMap.Category] as GoalCategory) || GoalCategory.OTHER,
        priority: (row[columnMap.Priority] as Priority) || Priority.MEDIUM,
        isActive: parseBoolean(row[columnMap['Is Active']]),
        createdAt: row[columnMap['Created At']] || new Date().toISOString(),
        updatedAt: row[columnMap['Updated At']] || new Date().toISOString()
 ...
 */
function mapCSVColumns(headers: string[] | undefined, expectedColumns: string[]): Record<string, number> {
  const columnMap: Record<string, number> = {};
  
  if (headers) {
    expectedColumns.forEach((column, defaultIndex) => {
      const headerIndex = headers.findIndex(h => h.trim().toLowerCase() === column.toLowerCase());
 ...
 */
function validateImportedUserPlan(userPlan: any): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
...
export function detectFileFormat(content: string): 'json' | 'csv' | 'unknown' {
  ...
  if (trimmed.includes(',') || trimmed.includes('\n')) {
    ...
    if (trimmed.includes('# Finance Planner Export') || 
